...
| Data statistik provinsi kosong ("Data X belum tersedia") di server baru padahal ada di local | Statistik disimpan sebagai file JSON cache (`storage/app/private/simkopdes/statistics.json`), bukan di database — file ini tidak ikut ke-clone dari GitHub | Jalankan manual: `node scripts/sync-simkopdes.mjs --region=KODE_PROVINSI` (kode provinsi sesuai `province_code` di Pengaturan Website) |
| `node scripts/sync-simkopdes.mjs` gagal dengan `Error: read ECONNRESET` di `Pipe.onStreamRead` | Chrome/Puppeteer crash karena dijalankan sebagai `root` tanpa `--no-sandbox` (default semua VPS aaPanel) | Sudah otomatis ditangani di `sync-simkopdes.mjs` (`--no-sandbox`, `--disable-setuid-sandbox`, `--disable-dev-shm-usage`) — kalau masih terjadi, update dulu ke versi terbaru dari repo |
| Halaman (terutama login admin) tampil polos tanpa styling + console error `filamentFormButton is not defined` / `isProcessing is not defined` dll | Cache Cloudflare atau browser menyimpan response JS (Livewire/Filament) yang rusak dari percobaan deploy sebelumnya | Coba buka di jendela Incognito dulu (cek cache browser lokal); kalau masih rusak, purge cache Cloudflare (Dashboard > Caching > Configuration > Purge Everything) |
